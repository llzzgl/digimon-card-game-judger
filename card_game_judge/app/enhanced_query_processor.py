...
提取关键信息：效果时机、触发条件、卡牌效果、处理顺序相关术语
"""
import re
...
class EnhancedQueryProcessor:
    ...
    def extract_card_numbers(self, text: str) -> List[str]:
        """提取所有卡牌编号并标准化"""
        matches = self.CARD_NO_PATTERN.findall(text)
        result = []
        seen = set()
        
        for m in matches:
            normalized = self._normalize_card_number(m)
    ...
    def _normalize_card_number(self, card_no: str) -> str:
        """
        标准化卡牌编号格式
        ...
        # 格式4: その他-001
        match = re.match(r'^その他-?(\d{1,3})$', card_no)
        if match:
            card_num = match.group(1).zfill(3)
            return f"その他-{card_num}"
    ...
    def build_enhanced_queries(self, query: str) -> List[Tuple[str, str, float]]:
        ...
        for card_no in concepts["card_numbers"]:
            queries.append((card_no, "card", 1.5))  # 高权重
